...
def copy_tga_files(directory):
    ...
    for filename in os.listdir(target_dir):
        ...
        if len(filename) == 7 and filename.lower().endswith('.tga'):
            base_name = filename[:3]  # Extract the 3-character name
            original_file = os.path.join(target_dir, filename)
            
            # Create the three copies with new names
            copy1 = os.path.join(target_dir, f"{base_name}_dominion.tga")
            copy2 = os.path.join(target_dir, f"{base_name}_dominion2.tga")
            copy3 = os.path.join(target_dir, f"{base_name}_dominion3.tga")
